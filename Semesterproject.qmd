...
stefan <- read_delim("posmo_stefan_230618.csv", delim = ",")
stefan$person <- "Stefan"

# Daten von Miriam (ab xx.05.2023)
miriam <- read_delim("posmo_stefan_230525.csv", delim = ",")
...
# IDEE: Damit wir auch Events finden, wo wir kein stay von 30 min haben (Spazieren, Kartieren,...), könnten wir über langen slow eine Zeile mit stay einfügen. Wie kann man Zeile einfügen???
...
# Buffer um Bahnhof
haltestelle_buffer <- st_buffer(haltestelle, 120) # Was ist eine gute Buffer-Grösse???
...
# Visualisierung: Wer hat wo wurde gewartet
ggplot(velocity)+
  geom_sf(aes(colour = person))+
  theme_void() # Achsen und Raster verschwinden

# Visualisierung: War die Wartezeit unterschiedlich?
ggplot(velocity)+
  geom_sf(aes(size = wartezeit))+
  theme_void() # Achsen und Raster verschwinden
...
# Barchart, Saeulendiagramm
ggplot(velocity)+
  geom_bar(aes(x = wartezeit))

ggplot(velocity)+
  geom_bar(aes(x = wartezeit, fill=person))
...
#, ylab = "Wartezeit [min]", xlab = "Haltestelle")
...
# Idee: Karte mit Boxplots pro Haltestelle für Wartezeit
