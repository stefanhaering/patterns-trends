...
posmo <- posmo |> # Zeit
  mutate(
    time = as.numeric(difftime(datetime, lag(datetime), units = "secs")) # secs
  ) # Hier gibt es manchmal 0er, also Datenpunkte zur selben Zeit. Die müssten auch selbe Koordinaten haben und sind für Geschwindigkeitsrechnung nicht zu gebrauchen. Also weg damit... Hier macht "lag" mehr Sinn, da so der Standort des Bahnhofs besser gefunden werden kann
...
posmo <- posmo |>
  mutate(
    steplength = round(sqrt((lag(X,1)-X)^2+(lag(Y,1)-Y)^2),1), # meter
...
event |> 
  mutate(wartezeit = as.numeric(difftime(ende_warten, start_warten, units = "mins"))) # das ist eher unwahrscheinlich... shit... müssten wir für die Geschwindigkeit doch lead nehmen?
